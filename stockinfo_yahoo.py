...
class StockInfo:
    ...
    def getStockLinks(self, url):
        ...
        for link in bsObj.findAll("a", href=re.compile('.*'+stock+'.*')): #Find a tags who href attribute have our stock in it
            ...
            if 'cnn' in url: #Formating for CNN links - there href come out differently
                link = 'http://cnn.com'+link #Simply adding to the front of the link
                if link not in self.internal_links:
                    self.internal_links.append(link)
            else:
                if link.startswith(url) and link not in self.internal_links: #Formatting for MW and YAHOO
                    self.internal_links.append(link)
