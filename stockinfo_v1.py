...
class StockInfo:
    def __init__(self, url, stock):
        '''
        Parameters: URL - Site where to grab data
    ...
    def getStockLinks(self):
        ...
        '''
        site = self.url #not neccessary but assign self.url to a variable
        stock = self.stock
        self.internal_links = [] 
        html = urlopen(site)
        bsObj = BeautifulSoup(html) #convert html to beautiful soup
        for link in bsObj.findAll("a", href=re.compile('.*'+stock+'.*')):
            link = link.attrs['href'] #find the href attrbutes in the a tag
    ...
    def send_mail(self, email, password):
        ...
        '''
        dataframe = self.links_df() #Main dataframe
    ...
    """
        ...
        mail.sendmail(me, you, msg.as_string()) #Conver to string datatype
        mail.quit()

def main():
    stock = input("Please give a stock to query: ")
    url = input("Please give a url to search: ")
    stockinfo = StockInfo(url,stock) #Initiate your class
